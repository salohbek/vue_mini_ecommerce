...
const actions = {
    ...
    checkoutCart({commit},cartItem){
        axios.post('/api/cart/checkout',cartItem).then((response)=>{
...
cartTotal:(state)=>{
    ...
    .reduce((acc,cartItems)=>{
        return cartItems.quantity*cartItems.price+acc
    },0)
    .toFixed(2)
},
cartQuantity:(state)=>{
